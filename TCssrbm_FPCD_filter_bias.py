...
def main_inpaint(filename, algo='Gibbs', rng=[777888,43,435,678,888], scale_separately=False, sampling_for_v=True, gibbs_steps=1001):
    ...
    shp = B_test.test_img[0].shape
    test_img_rows, test_img_cols = shp
    img = numpy.zeros((1,)+shp)
    temp_img = numpy.asarray(B_test.test_img[0], dtype='uint8')
    img[0,] = temp_img
	...
	scaled_batchdata_center[:,:,:,:] = batchdata[:,:,11:65,11:65]   
    
        batchdata[:,:,11:65,11:65] = 0
    ...
    results_f.write('%f, %f\n'%(value_TSS_inpainted_test.mean(),value_TSS_inpainted_test.std())) 
    results_.close()
...
def main_train(argv):
    ...
    conf['data_rescale'] = float(argv[3]) #2.0;1.0;1.333333
    directory_name = directory_name + 'resize' + argv[3]
    directory_name += '_'
